...
//
#include "include/face_weights.h"
...
void getFiles( const string& path, vector<string>& files, vector<string> &file_name)
{
    /*files存储文件的路径及名称
     file_name只存储文件的名称*/
    ...
    if((hFile = _findfirst(p.assign(path).append("\\*").c_str(),&file_info)) !=  -1)
    {
        do
        {
            ...
            else
            {
                ...
                file_name.emplace_back(file_info.name);
            }
...
int main() {
    ...
    getFiles( imgPath, files, file_name);
    //程序预热
    for (int j = 0; j < 3; j++) {
    ...
    for (int i = 0; i < files.size(); ++i) {
        ...
        float *out = GetFaceScore(img);
        cout << "--------------------------" << endl;
        cout << "File name: " << file_name[i] << endl;
